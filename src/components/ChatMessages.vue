...
const loadMessages = async () => {
  try {
    const resp = await fetchMessages(props.chatId)
...
const sendMessage = async () => {
  ...
  try {
    const resp = await postMessage(props.chatId, content)
...
<template>
  <section class="flex flex-col h-full px-4 py-6">
    <!-- 消息列表 -->
    <div class="flex-1 overflow-y-auto space-y-4 pr-1">
      ...
      >
        <div
            ...
            'max-w-[70%] p-3 rounded-2xl shadow transition-colors duration-300'
          ]"
        >
    ...
    <!-- 输入框 -->
    <form @submit.prevent="sendMessage" class="mt-4 flex items-center">
      <input
          v-model="newMessage"
          type="text"
          ...
          'flex-1 px-4 py-2 rounded-l-full focus:outline-none focus:ring-2 focus:ring-amber-500 transition'
        ]"
      />
      <button
          type="submit"
          ...
          'px-6 py-2 rounded-r-full text-white font-medium transition-colors duration-300 focus:outline-none'
        ]"
      >
        Send
      </button>
